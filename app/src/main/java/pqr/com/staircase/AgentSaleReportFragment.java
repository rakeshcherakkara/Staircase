 ...
 */
public class AgentSaleReportFragment extends Fragment {
    ...
    String agentSaleReportUrl = "http://192.168.1.8:8000/api/agentsalereport";



    public AgentSaleReportFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_agent_sale_report, container, false);
                ...
                StringRequest salereportRequest = new StringRequest(Request.Method.POST, agentSaleReportUrl, new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            JSONArray agentsale_array = agentsale_object.getJSONArray("agent_salereport");
                            int agentsale_length = agentsale_array.length();
                            JSONArray subagentsale_array = agentsale_object.getJSONArray("subagent_salereport");
                            int subagentsale_length = agentsale_array.length();
                            ...
                            if(subagentsale_length!=0)
                            {

                                for(int i=0;i<subagentsale_length;i++)
                                {
                                    String sale_date = subagentsale_array.getJSONObject(i).getString("date");

//                                    agentSaleReportDataProvider = new AgentSaleReportDataProvider("","",sale_date,"","");
//                                    agentSaleReportAdapter.add(agentSaleReportDataProvider);


                                    JSONArray bill_array = subagentsale_array.getJSONObject(i).getJSONArray("bills");
                                    int bill_length = bill_array.length();
                                    ...
                                    if(bill_length!=0)
                                    {
                                        ...
                                        {
                                            String bill_number = bill_array.getJSONObject(j).getString("bill_number");
                                            String customer = bill_array.getJSONObject(j).getString("subagent_name");
                                            String total_quantity = bill_array.getJSONObject(j).getString("total_quantity");
                                            String grand_total = bill_array.getJSONObject(j).getString("grand_total");
                                            ...
                                            if(finalConsolidate.equals("no"))
                                            {
                                                ...
                                                if(tickets_length!=0)
                                                {
                                                    for(int k=0;k<tickets_length;k++)
                                                    {
                                                        ...
                                                        String number = tickets_array.getJSONObject(k).getString("number");
                                                        String quantity = tickets_array.getJSONObject(k).getString("quantity");
                                                        String total = tickets_array.getJSONObject(k).getString("agent_total");
                ...
                })
                {
                    ...
                    protected Map<String, String> getParams() throws AuthFailureError {
                        ...
                        params.put("customer_name", customer_1);
